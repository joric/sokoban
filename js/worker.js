...
rtl.module("System",[],function () {
  ...
  this.Writeln = function () {
    ...
    if ($impl.WriteCallBack != null) {
      ...
      //console.log(s);
      //do not console log, put into a buffer and return on proc_exit
      rtl.output.push(s.trim()); // trim \n
...
rtl.module("browserconsole",["System","JS","Web","Rtl.BrowserLoadHelper","SysUtils"],function () {
  ...
  this.HookConsole = function () {
    // NB! disable writing to html page
    /*
...
rtl.module("wasienv",["System","JS","webassembly","Types"],function () {
  "use strict";
  var $mod = this;
  ...
  rtl.createClass(this,"TPas2JSWASIEnvironment",pas.System.TObject,function () {
    ...
    this.args_get = function(argv, argvBuf) {
      var Result = 0;
      var argvArray = rtl.argv;
      var argc = argvArray.length;
      var argvPtr = argvBuf;
      var View = this.getModuleMemoryDataView();
      var offset = argvPtr;
      for (var i = 0; i < argc; i++) {
        var argStr = argvArray[i];
        var argStrLen = argStr.length;
        View.setUint32(argv + i * 4, offset, this.FIsLittleEndian);
        for (var j = 0; j < argStrLen; j++) {
          View.setUint8(offset++, argStr.charCodeAt(j));
        }
    ...
    this.args_sizes_get = function(argc, argvBufSize) {
      var Result = 0;
      var argvArray = rtl.argv;
      var argcValue = argvArray.length;
      var View = null;
      View = this.getModuleMemoryDataView();
      var argvBufSizeValue = 0;
      for (var i = 0; i < argcValue; i++) {
        var argStr = argvArray[i];
        var argStrLen = argStr.length;
        argvBufSizeValue += 4;
        argvBufSizeValue += argStrLen;
        argvBufSizeValue += 1;
      }
      argvBufSizeValue = (argvBufSizeValue + 3) & ~3;
      View.setUint32(argc, argcValue, this.FIsLittleEndian);
      View.setUint32(argvBufSize, argvBufSizeValue, this.FIsLittleEndian);
      Result = 0;
      return Result;
    };
    ...
    this.fd_write = function (fd, iovs, iovsLen, nwritten) {
      ...
      function getiovs(iovs, iovsLen) {
        ...
        for (var $l = 0, $end = iovsLen - 1; $l <= $end; $l++) {
          I = $l;
          Ptr = iovs + (I * 8);
          Buf = view.getUint32(Ptr,$Self.FIsLittleEndian);
          BufLen = view.getUint32(Ptr + 4,$Self.FIsLittleEndian);
          ArrayBuf = new Uint8Array($Self.FModuleInstanceExports.memory.buffer,Buf,BufLen);
    ...
    this.proc_exit = function (rval) {
      var Result = 0;
      Result = 52;
      rtl.callback && rtl.callback(rtl.output);
      return Result;
    };
...
rtl.module("program",["System","browserconsole","browserapp","JS","Classes","SysUtils","Web","webassembly","Types","wasienv"],function () {
  "use strict";
  var $mod = this;
  rtl.createClass(this,"TMyApplication",pas.browserapp.TBrowserApplication,function () {
    ...
    this.CreateWebAssembly = function (Path, ImportObject) {
      var $Self = this;
      var Result = null;
      Result = fetch(Path).then(function (res) {
        var Result = undefined;
        Result = res.arrayBuffer().then(function (res2) {
          var Result = undefined;
          Result = WebAssembly.instantiate(res2,ImportObject);
          return Result;
        },null);
        return Result;
      },null);
    ...
    this.initEnv = function (aValue) {
      var Result = undefined;
      var exps = null;
      Result = true;
      exps = aValue.instance.exports;
      this.FWasiEnv.SetInstance(aValue.instance);
      //window.console.info("got exports",exps);
      exps._start();
      return Result;
    };
    this.InitWebAssembly = function () {
      var $Self = this;
      var mDesc = pas.webassembly.TJSWebAssemblyMemoryDescriptor.$new();
      var tDesc = pas.webassembly.TJSWebAssemblyTableDescriptor.$new();
      var aTable = null;
      var ImportObj = null;
      mDesc.initial = 256;
      mDesc.maximum = 256;
      this.FMemory = new WebAssembly.Memory(pas.webassembly.TJSWebAssemblyMemoryDescriptor.$clone(mDesc));
      tDesc.initial = 0;
      tDesc.maximum = 0;
      tDesc.element = "anyfunc";
      aTable = new WebAssembly.Table(pas.webassembly.TJSWebAssemblyTableDescriptor.$clone(tDesc));
      ImportObj = pas.JS.New(["wasi_unstable",this.FWasiEnv.GetImportObject(),"wasi_snapshot_preview1",this.FWasiEnv.GetImportObject(),"env",pas.JS.New(["abortStackOverflow",function () {
        throw pas.SysUtils.Exception.$create("Create$1",["overflow"]);
      },"table",aTable,"tableBase",0,"memory",this.FMemory,"memoryBase",1024,"STACKTOP",0,"STACK_MAX",this.FMemory.buffer.byteLength]),"js",pas.JS.New(["mem",this.FMemory])]);
      this.CreateWebAssembly("YASS.wasm",ImportObj).then(rtl.createCallback($Self,"initEnv"));
    };
    this.Create$1 = function (aOwner) {
...
// Set up the worker to handle messages from the main thread
onmessage = async (event) => {
  const { args } = event.data;
  try {
    // Prepare the WASM module and configuration
    rtl.argv = args;
    rtl.output = [];
    rtl.callback = (result) => {
    ...
    // Run the WASM module (assuming rtl.run() is synchronous)
    rtl.run();
  } catch (error) {
